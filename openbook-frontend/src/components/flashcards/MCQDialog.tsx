...
import type { MCQSession } from "@/store/openBookStore";
...
export function MCQDialog({ open, onOpenChange, session }: MCQDialogProps) {
  ...
  const handleReset = () => {
    setQuestions(session.questions.map((q) => ({ ...q, selectedIndex: null })));
  };
